	...
	<input type="hidden" name="num" value="<%=dto.getNum()%>">
		<table class="table table-bordered">
				...
				<td>
					<select name="age" class="form-control" style="width: 200px;">
						...
						for(int i=0;i<str.length;i++)
						{
							...
							<%}else{%>
								<option value="<%=str[i] %>"><%=str[i].equals("기타")?str[i]:str[i]+"대" %></option>	
							<%}
						}
						%>
					</select>
			...
			<tr>
				<th>가능언어</th>
				<td>
					<input type="checkbox" name="hobby1" value="영어"
					<%=dto.getLang().contains("영어")?"checked":"" %>>영어&nbsp;
					<input type="checkbox" name="hobby1" value="일본어"
					<%=dto.getLang().contains("일본")?"checked":"" %>>일본어&nbsp;
					<input type="checkbox" name="hobby1" value="중국어"
					<%=dto.getLang().contains("중국")?"checked":"" %>>중국어&nbsp;
					<input type="checkbox" name="hobby1" value="스페인어"
					<%=dto.getLang().contains("스페인")?"checked":"" %>>스페인어&nbsp;
					<input type="checkbox" name="hobby1" value="베트남어"
					<%=dto.getLang().contains("베트남")?"checked":"" %>>베트남어&nbsp;
					<input type="checkbox" name="hobby1" value="불어"
